...
void Server::handle_client_open(MDRequestRef& mdr)
{
  ...
  char* filename_corre;
  int flag_have_correlation = corefs_get_correlations(cur, &filename_corre);
  dout(7) << __func__ << " flag = " << flag_have_correlation << " filename_corre = " << filename_corre << dendl;
  if(flag_have_correlation){
    CInode *cin_prefetched = corefs_prefetch_cinode(filename_corre);
    dout(7) << __func__ << *cin_prefetched << dendl;
  }
