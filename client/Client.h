...
public:
  ...
  ceph::unordered_map<uint64_t,int> inode_times_phd;
  void show_table_feedback();
  void show_access_sequence();
  void corefs_access(uint64_t ino, bool prefetched);
  void corefs_update_feedback(uint64_t ino);
  void corefs_send_feedback(MetaRequest* request);
  void mon_op_init();
  static void* count_iops_thread(void* args);
  static void* adaptive_feedback(void* args);
  void corefs_umount_flush_feedback();
  int correlation_overlap(long offset1, int len1, long offset2, int len2);
