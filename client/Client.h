...
public:
  ...
  // manipulate data correlations
  void speedymeta_set_dc(Inode *in, string key, bufferptr bp);
  void speedymeta_rm_dc(Inode *in, string key);
  void speedymeta_list_dc(Inode *in);
  void speedymeta_mark_dc_dirty(Inode *in);
  void speedymeta_cache_dc(Inode *in, string key, void* value, uint64_t size);

  // overwrite content
  int speedymeta_dc_overlap(uint64_t offset, uint64_t len, uint64_t dc_offset, size_t dc_len);
  void speedymeta_overwrite(Inode *in, uint64_t offset, uint64_t size);

  // receive mds reply
  int speedymeta_parse_reply(bufferlist::iterator p, MetaRequest *request, MClientReply *reply,  MetaSession *session, Inode* in);

  vector<uint64_t> access_sequence;
  std::unordered_map<uint64_t, std::unordered_map<uint64_t, struct feedback_info*>> table_feedback;
  float total_phd, total_phd_access;
  float period_phd, period_phd_access;
  #ifdef _SPEEDYMETA_FEEDBACK
  void speedymeta_show_table_feedback();
  void speedymeta_show_access_sequence();
  void speedymeta_update_feedback(uint64_t ino);
  void speedymeta_access(uint64_t ino, bool prefetched);
  void speedymeta_send_feedback(MetaRequest* request);
  static void* speedymeta_adaptive_feedback(void* args);
  #endif
};
