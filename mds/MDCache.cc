                           ...
                           int onfail)
{
  ...
  while (depth < path.depth()) {
	...
	*pin = cur;
    ...
    std::map<dentry_key_t, CDentry*>::iterator it = curdir->items.begin();
    int tmp_num = 0;
    while(it != curdir->items.end()){
      dout(2) << " [Debug-dentry-list] " << ++tmp_num << " " << it->second->name << dendl;
      it++;
    }
    dout(2) << "[Debug] dir->complete=" << curdir->is_complete() << dendl;
    dout(2) << "[Debug] snapid == CEPH_NOSNAP ? " << snapid << dendl;
    dout(2) << "[Debug] dir->has_bloom() = " << curdir->has_bloom() << dendl;
    dout(2) << "[Debug] dir->is_in_bloom() = " << curdir->is_in_bloom(path[depth]) << dendl;
    if (curdir->is_auth()) {
      // dentry is mine.
      if (curdir->is_complete() ||
	  (snapid == CEPH_NOSNAP &&
