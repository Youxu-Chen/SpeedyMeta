...
void Server::reply_client_request(MDRequestRef& mdr, MClientReply *reply)
{
  ...
  if (!did_early_reply && !is_replay) {
    ...
    if(tracei_phd.size() > 0){
      for(vector<CInode*>::iterator it = tracei_phd.begin(); it != tracei_phd.end(); it++)
        mdr->cap_releases.erase((*it)->vino());
      for(vector<vector<CDentry*>>::iterator iter = tracedn_phd.begin(); iter != tracedn_phd.end(); iter++)
        for(vector<CDentry*>::iterator it = (*iter).begin(); it != (*iter).end(); it++)
          mdr->cap_releases.erase((*it)->get_dir()->get_inode()->vino());
    }
    #endif
  }
...
void Server::dispatch_client_request(MDRequestRef& mdr)
{
  ...
  case CEPH_MDS_OP_FEEDBACK:
    speedymeta_handle_client_feedback(mdr);
    break;
  #endif
