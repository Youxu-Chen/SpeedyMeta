...
void Server::reply_client_request(MDRequestRef& mdr, MClientReply *reply)
{
  ...
  // corefs- get tracei_prefetched from mdr
#ifdef COREFS_PREFETCH
  vector<CInode*> tracei_prefetched = mdr->tracei_prefetched;
  vector<vector<CDentry*>> dn_prefetched = mdr->tracedn_prefetched;
#endif
  ...
  if (!did_early_reply && !is_replay) {
    ...
    // corefs
#ifdef COREFS_PREFETCH
    if(tracei_prefetched.size() > 0){
      for(vector<CInode*>::iterator p = tracei_prefetched.begin(); p != tracei_prefetched.end(); p++)
        mdr->cap_releases.erase((*p)->vino());
      for(vector<vector<CDentry*>>::iterator iter = dn_prefetched.begin(); iter != dn_prefetched.end(); iter++)
        for(vector<CDentry*>::iterator v_iter = (*iter).begin(); v_iter != (*iter).end(); v_iter++ )
          mdr->cap_releases.erase((*v_iter)->get_dir()->get_inode()->vino());
    }
#endif
  }
    ...
    reply->head.is_target = 1;
      ...
      for(vector<vector<CDentry*>>::iterator iter = mdr->tracedn_prefetched.begin(); iter != mdr->tracedn_prefetched.end(); iter++, i++){
        ...
        for(vector<CDentry*>::iterator v_iter = (*iter).begin(); v_iter != (*iter).end(); v_iter++){
          ...
          diri_prefetched = dir_prefetched->get_inode();
          diri_prefetched->encode_inodestat(bl, session, NULL, snapid);
          dout(2) << "corefs_set_trace_dist added diri " << *diri_prefetched << dendl;
          #ifdef MDS_VERIFY_FRAGSTAT
            if(dir_prefetched->is_complete())
              dir_prefetched->verify_fragstat();
          #endif
          dir_prefetched->encode_dirstat(bl, whoami);
          // dout(2) << "corefs_set_trace_dist added dir " << *dir_prefetched << dendl;

          ::encode((*v_iter)->get_name(), bl);
          dout(2) << "corefs_set_trace_dist added dentry name " << (*v_iter)->get_name() << dendl;
          if(snapid == CEPH_NOSNAP)
            mds->locker->issue_client_lease((*v_iter), client, bl, now, session);
          else
            encode_null_lease(bl);
          dout(2) << "corefs_set_trace_dist added dn " << snapid << " " << *(*v_iter) << dendl;
        }
...
void Server::dispatch_client_request(MDRequestRef& mdr)
{
  ...
  case CEPH_MDS_OP_LOOKUPINO:
    ...
    dout(2) << "-handle_client_lookup_ino-" << " (start, OP_LOOKUPINO, path." << req->get_filepath() << ")" << dendl;
  ...
  case CEPH_MDS_OP_LOOKUP:
    mon_req[OP_LOOKUP]++;
    dout(2) << "-handle_client_getattr-" << " (start, OP_LOOKUP, path." << req->get_filepath() << ")" << dendl;
  ...
  case CEPH_MDS_OP_GETATTR:
    mon_req[OP_GETATTR]++;
    dout(2) << "-handle_client_getattr-" << " (start, OP_GETATTR, path." << req->get_filepath() << ")" << dendl;
    handle_client_getattr(mdr, false);
    break;

  case CEPH_MDS_OP_SETATTR:
    mon_req[OP_SETATTR]++;
    dout(2) << "-handle_client_setattr-" << " (start, OP_SETATTR, path." << req->get_filepath() << ")" << dendl;
    handle_client_setattr(mdr);
    break;
  case CEPH_MDS_OP_SETLAYOUT:
    mon_req[OP_SETLAYOUT]++;
    dout(2) << "-handle_client_setlayout-" << " (start, OP_SETLAYOUT, path." << req->get_filepath() << ")" << dendl;
    handle_client_setlayout(mdr);
    break;
  case CEPH_MDS_OP_SETDIRLAYOUT:
    mon_req[OP_SETDIRLAYOUT]++;
    dout(2) << "-handle_client_setdirlayout-" << " (start, OP_SETLAYOUT, path." << req->get_filepath() << ")" << dendl;
    handle_client_setdirlayout(mdr);
    break;
  case CEPH_MDS_OP_SETXATTR:
    dout(2) << "-handle_client_setxattr-" << " (start, OP_SETXATTR, path." << req->get_filepath() << ")" << dendl;
    mon_req[OP_SETXATTR]++;
    handle_client_setxattr(mdr);
    break;
  case CEPH_MDS_OP_RMXATTR:
    dout(2) << "-handle_client_removexattr-" << " (start, OP_RMXATTR, path." << req->get_filepath() << ")" << dendl;
    mon_req[OP_RMXATTR]++;
    handle_client_removexattr(mdr);
    break;

  case CEPH_MDS_OP_READDIR:
    dout(2) << "-handle_client_readdir-" << " (start, OP_READDIR, path." << req->get_filepath() << ")" << dendl;
    mon_req[OP_READDIR]++;
    handle_client_readdir(mdr);
    break;

  case CEPH_MDS_OP_SETFILELOCK:
    dout(2) << "-handle_client_file_setlock-" << " (start, OP_SETFILELOCK, path." << req->get_filepath() << ")" << dendl;
    mon_req[OP_SETFILELOCK]++;
    handle_client_file_setlock(mdr);
    break;

  case CEPH_MDS_OP_GETFILELOCK:
    dout(2) << "-handle_client_file_readlock-" << " (start, OP_GETFILELOCK, path." << req->get_filepath() << ")" << dendl;
  ...
  case CEPH_MDS_OP_CREATE:
    if (mdr->has_completed){
      mon_req[OP_OPEN]++;
      dout(2) << "-handle_client_open-" << " (start, OP_OPEN, path." << req->get_filepath() << ")" << dendl;
      handle_client_open(mdr);  // already created.. just open
    }
    else{
      dout(2) << "-handle_client_openc-" << " (start, OP_CREATE, path." << req->get_filepath() << ")" << dendl;
      mon_req[OP_CREATE]++;
  ...
  case CEPH_MDS_OP_MKNOD:
    dout(2) << "-handle_client_mknod-" << " (start, OP_MKNOD, path." << req->get_filepath() << ")" << dendl;
    mon_req[OP_MKNOD]++;
    handle_client_mknod(mdr);
    break;
  case CEPH_MDS_OP_LINK:
    dout(2) << "-handle_client_link-" << " (start, OP_LINK, path." << req->get_filepath() << ")" << dendl;
    mon_req[OP_LINK]++;
    handle_client_link(mdr);
    break;
  case CEPH_MDS_OP_UNLINK:
  case CEPH_MDS_OP_RMDIR:
    dout(2) << "-handle_client_unlink-" << " (start, OP_UNLINK, path." << req->get_filepath() << ")" << dendl;
    mon_req[OP_UNLINK]++;
    handle_client_unlink(mdr);
    break;
  case CEPH_MDS_OP_RENAME:
    dout(2) << "-handle_client_rename-" << " (start, OP_RENAME, path." << req->get_filepath() << ")" << dendl;
    mon_req[OP_RENAME]++;
    handle_client_rename(mdr);
    break;
  case CEPH_MDS_OP_MKDIR:
    dout(2) << "-handle_client_mkdir-" << " (start, OP_MKDIR, path." << req->get_filepath() << ")" << dendl;
    mon_req[OP_MKDIR]++;
    handle_client_mkdir(mdr);
    break;
  case CEPH_MDS_OP_SYMLINK:
    dout(2) << "-handle_client_symlink-" << " (start, OP_SYMLINK, path." << req->get_filepath() << ")" << dendl;
    mon_req[OP_SYMLINK]++;
  ...
  case CEPH_MDS_OP_LSSNAP:
    dout(2) << "-handle_client_lssnap-" << " (start, OP_LSSNAP, path." << req->get_filepath() << ")" << dendl;
    mon_req[OP_LSSNAP]++;
    handle_client_lssnap(mdr);
    break;
  case CEPH_MDS_OP_MKSNAP:
    dout(2) << "-handle_client_mksnap-" << " (start, OP_MKSNAP, path." << req->get_filepath() << ")" << dendl;
    mon_req[OP_MKSNAP]++;
    handle_client_mksnap(mdr);
    break;
  case CEPH_MDS_OP_RMSNAP:
    dout(2) << "-handle_client_rmsnap-" << " (start, OP_RMSNAP, path." << req->get_filepath() << ")" << dendl;
    mon_req[OP_RMSNAP]++;
    handle_client_rmsnap(mdr);
    break;
  case CEPH_MDS_OP_RENAMESNAP:
    dout(2) << "-handle_client_renamesnap-" << " (start, OP_RENAMESNAP, path." << req->get_filepath() << ")" << dendl;
    mon_req[OP_RENAMESNAP]++;
    handle_client_renamesnap(mdr);
    break;
  case CEPH_MDS_OP_FEEDBACK: // corefs
    dout(2) << "-handle_client_feedback-" <<  dendl;
    corefs_handle_client_feedback(mdr);
    break;
  ...
  case CEPH_MDS_OP_LOOKUPHASH:
  case CEPH_MDS_OP_LOOKUPINO:
    dout(2) << "-handle_client_lookup_ino-" << " (end, OP_LOOKUPINO, path." << req->get_filepath() << ")" << dendl;
    mon_req[OP_LOOKUPINO]--;
    mon_op[OP_LOOKUPINO]++;
    break;
  case CEPH_MDS_OP_LOOKUPPARENT:
  ...
  case CEPH_MDS_OP_LOOKUPNAME:
    ...
    // inodes ops.
  case CEPH_MDS_OP_LOOKUP:
    dout(2) << "-handle_client_getattr-" << " (end, OP_LOOKUP, path." << req->get_filepath() << ")" << dendl;
    mon_req[OP_LOOKUP]--;
    mon_op[OP_LOOKUP]++;
    break;
  ...
  case CEPH_MDS_OP_LOOKUPSNAP:
    // lookupsnap does not reference a CDentry; treat it as a getattr
  case CEPH_MDS_OP_GETATTR:
    dout(2) << "-handle_client_getattr-" << " (end, OP_GETATTR, path." << req->get_filepath() << ")" << dendl;
    mon_req[OP_GETATTR]--;
    mon_op[OP_GETATTR]++;
    break;

  case CEPH_MDS_OP_SETATTR:
    dout(2) << "-handle_client_setattr-" << " (end, OP_SETATTR, path." << req->get_filepath() << ")" << dendl;
    mon_req[OP_SETATTR]--;
    mon_op[OP_SETATTR]++;
    break;
  case CEPH_MDS_OP_SETLAYOUT:
    dout(2) << "-handle_client_setlayout-" << " (end, OP_SETLAYOUT, path." << req->get_filepath() << ")" << dendl;
    mon_req[OP_SETLAYOUT]--;
    mon_op[OP_SETLAYOUT]++;
    break;
  case CEPH_MDS_OP_SETDIRLAYOUT:
    dout(2) << "-handle_client_setdirlayout-" << " (end, OP_SETLAYOUT, path." << req->get_filepath() << ")" << dendl;
    mon_req[OP_SETDIRLAYOUT]--;
    mon_op[OP_SETDIRLAYOUT]++;
    break;
  case CEPH_MDS_OP_SETXATTR:
    dout(2) << "-handle_client_setxattr-" << " (end, OP_SETXATTR, path." << req->get_filepath() << ")" << dendl;
    mon_req[OP_SETXATTR]--;
    mon_op[OP_SETXATTR]++;
    break;
  case CEPH_MDS_OP_RMXATTR:
    dout(2) << "-handle_client_removexattr-" << " (end, OP_RMXATTR, path." << req->get_filepath() << ")" << dendl;
    mon_req[OP_RMXATTR]--;
    mon_op[OP_RMXATTR]++;
    break;

  case CEPH_MDS_OP_READDIR:
    dout(2) << "-handle_client_readdir-" << " (end, OP_READDIR, path." << req->get_filepath() << ")" << dendl;
    mon_req[OP_READDIR]--;
    mon_op[OP_READDIR]++;
    break;

  case CEPH_MDS_OP_SETFILELOCK:
    dout(2) << "-handle_client_file_setlock-" << " (end, OP_SETFILELOCK, path." << req->get_filepath() << ")" << dendl;
    mon_req[OP_SETFILELOCK]--;
    mon_op[OP_SETFILELOCK]++;
    break;

  case CEPH_MDS_OP_GETFILELOCK:
    dout(2) << "-handle_client_file_readlock-" << " (end, OP_GETFILELOCK, path." << req->get_filepath() << ")" << dendl;
    mon_req[OP_GETFILELOCK]--;
  ...
  case CEPH_MDS_OP_CREATE:
    if (mdr->has_completed){
      dout(2) << "-handle_client_open-" << " (end, OP_OPEN, path." << req->get_filepath() << ")" << dendl;
      mon_req[OP_OPEN]--; // already created.. just open
      mon_op[OP_OPEN]++;
    }
    else{
      dout(2) << "-handle_client_openc-" << " (end, OP_CREATE, path." << req->get_filepath() << ")" << dendl;
      mon_req[OP_CREATE]--;
      mon_op[OP_CREATE]++;
    }
    break;
  ...
  case CEPH_MDS_OP_OPEN:
    ...
    // no prior locks.
  case CEPH_MDS_OP_MKNOD:
    dout(2) << "-handle_client_mknod-" << " (end, OP_MKNOD, path." << req->get_filepath() << ")" << dendl;
    mon_req[OP_MKNOD]--;
    mon_op[OP_MKNOD]++;
    break;
  case CEPH_MDS_OP_LINK:
    dout(2) << "-handle_client_link-" << " (end, OP_LINK, path." << req->get_filepath() << ")" << dendl;
    mon_req[OP_LINK]--;
    mon_op[OP_LINK]++;
    break;
  case CEPH_MDS_OP_UNLINK:
  case CEPH_MDS_OP_RMDIR:
    dout(2) << "-handle_client_unlink-" << " (end, OP_UNLINK, path." << req->get_filepath() << ")" << dendl;
    mon_req[OP_UNLINK]--;
    mon_op[OP_UNLINK]++;
    break;
  case CEPH_MDS_OP_RENAME:
    dout(2) << "-handle_client_rename-" << " (end, OP_RENAME, path." << req->get_filepath() << ")" << dendl;
    mon_req[OP_RENAME]--;
    mon_op[OP_RENAME]++;
    break;
  case CEPH_MDS_OP_MKDIR:
    dout(2) << "-handle_client_mkdir-" << " (end, OP_MKDIR, path." << req->get_filepath() << ")" << dendl;
    mon_req[OP_MKDIR]--;
    mon_op[OP_MKDIR]++;
    break;
  case CEPH_MDS_OP_SYMLINK:
    dout(2) << "-handle_client_symlink-" << " (end, OP_SYMLINK, path." << req->get_filepath() << ")" << dendl;
    mon_req[OP_SYMLINK]--;
    mon_op[OP_SYMLINK]++;
    break;
    ...
    // snaps
  case CEPH_MDS_OP_LSSNAP:
    dout(2) << "-handle_client_lssnap-" << " (end, OP_LSSNAP, path." << req->get_filepath() << ")" << dendl;
    mon_req[OP_LSSNAP]--;
    mon_op[OP_LSSNAP]++;
    break;
  case CEPH_MDS_OP_MKSNAP:
    dout(2) << "-handle_client_mksnap-" << " (end, OP_MKSNAP, path." << req->get_filepath() << ")" << dendl;
    mon_req[OP_MKSNAP]--;
    mon_op[OP_MKSNAP]++;
    break;
  case CEPH_MDS_OP_RMSNAP:
    dout(2) << "-handle_client_rmsnap-" << " (end, OP_RMSNAP, path." << req->get_filepath() << ")" << dendl;
    mon_req[OP_RMSNAP]--;
    mon_op[OP_RMSNAP]++;
    break;
  case CEPH_MDS_OP_RENAMESNAP:
    dout(2) << "-handle_client_renamesnap-" << " (end, OP_RENAMESNAP, path." << req->get_filepath() << ")" << dendl;
    mon_req[OP_RENAMESNAP]--;
    mon_op[OP_RENAMESNAP]++;
    break;
  case CEPH_MDS_OP_FEEDBACK:
    break;

  default:
    dout(2) << " unknown client op " << req->get_op() << dendl;
    respond_to_request(mdr, -EOPNOTSUPP);
  }
}
				    ...
				    bool no_lookup)    // true if we cannot return a null dentry lease
{
  ...
  if (mdr->done_locking)
    return mdr->in[n];
  // corefs-time
  // utime_t find_start = ceph_clock_now(g_ceph_context);
  // traverse
  int r = mdcache->path_traverse(mdr, NULL, NULL, refpath, &mdr->dn[n], &mdr->in[n], MDS_TRAVERSE_FORWARD);
  // utime_t find_end = ceph_clock_now(g_ceph_context);
  // dout(1) << __func__ << " in path_traverse lat=" << find_end - find_start << "s." << dendl;
  if (r > 0)
    return NULL; // delayed
  if (r < 0) {  // error
...
void Server::handle_client_getattr(MDRequestRef& mdr, bool is_lookup)
{
  ...
  if (!ref) return;
  utime_t find_end = ceph_clock_now(g_ceph_context);
  // dout(2) << __func__ << " in rdblock_path_pin_ref lat=" << find_end - find_start << "s." << dendl;
  ...
  dout(2) << __func__ << " mask=" << mask << dendl;
  ...
  mds->balancer->hit_inode(ceph_clock_now(g_ceph_context), ref, META_POP_IRD,
   ...
   // prefetching when lookup a file
  #ifdef COREFS_PREFETCH
  #ifdef TIME_PHD
  utime_t phd_start = ceph_clock_now(g_ceph_context);
  #endif
  if(is_lookup && ref->is_file()){
    ...
    vector<string> files_deleted;
    
    std::vector<CDentry*>::iterator pdn;
    CInode *cin_prefetched;
    map<string, bufferptr> *core_px = ref->get_projected_xattrs();
    std::map<std::string, bufferptr>::iterator pxat = core_px->begin();
    std::string filename_corre;
    int max = MAX_PHD;
    set<SimpleLock*> core_rdlocks, core_wrlocks, core_xlocks;
    vector<int> corefs_phd_caps;
    int j = 0;
    // int max = max_phd_items;
    dout(2) << __func__ << " xattrs.size = " << core_px->size() << dendl;
    // if(core_px->size() > MAX_PHD)
    //   goto out;

    for(; pxat != core_px->end(); pxat++, j++){
        dout(2) << "[JJ]" << j << dendl;
       if(j >= max)
         break;
      ...
      if(filename_corre[0] != '/'){
        ...
        if(corefs_get_dc_score(ref, filename_corre) >= SCORE_THRESHOLD){
          ...
          if(r>0){
            dout(2) << __func__ << " missed inode" << dendl;
            return;
          }
          dout(2) << __func__ << " dn.size = " << dn_prefetched.size() << dendl;
          filepath fp_tmp;
          fp_tmp.set_path(filename_corre.c_str());
          dout(2) << "[Debug] cin_prefetched=" << *cin_prefetched << dendl;
          if (cin_prefetched != NULL && dn_prefetched.size() == fp_tmp.depth()){
            // if (cin_prefetched->is_file()){
              core_rdlocks.clear();
              core_wrlocks.clear();
              core_xlocks.clear();
              for (int i=0; i<(int)dn_prefetched.size(); i++) 
                core_rdlocks.insert(&dn_prefetched[i]->lock);
              
              mds->locker->include_snap_rdlocks(core_rdlocks, cin_prefetched);
              mdr->pin(cin_prefetched);
          
              int corefs_issued = 0;
              Capability *corefs_cap = cin_prefetched->get_client_cap(client);
              if (corefs_cap && (mdr->snapid == CEPH_NOSNAP ||
                    mdr->snapid <= corefs_cap->client_follows))
                corefs_issued = corefs_cap->issued();
              if ((mask & CEPH_CAP_LINK_SHARED) && (corefs_issued & CEPH_CAP_LINK_EXCL) == 0) core_rdlocks.insert(&cin_prefetched->linklock);
              if ((mask & CEPH_CAP_AUTH_SHARED) && (corefs_issued & CEPH_CAP_AUTH_EXCL) == 0) core_rdlocks.insert(&cin_prefetched->authlock);
              if ((mask & CEPH_CAP_FILE_SHARED) && (corefs_issued & CEPH_CAP_FILE_EXCL) == 0) core_rdlocks.insert(&cin_prefetched->filelock);
              if ((mask & CEPH_CAP_XATTR_SHARED) && (corefs_issued & CEPH_CAP_XATTR_EXCL) == 0) core_rdlocks.insert(&cin_prefetched->xattrlock);

              mdr->done_locking = false;
              if (!mds->locker->acquire_locks(mdr, core_rdlocks, core_wrlocks, core_xlocks)){
                dout(1) << __func__ << " [corefs] failed." << dendl;                
                continue;
              }

              if (!check_access(mdr, cin_prefetched, MAY_READ))
                continue;
              #ifdef COREFS_FEEDBACK
              corefs_add_dc_score(ref, filename_corre);
              #endif
            ...
            #ifdef COREFS_DELETE
            // file was deleted. Hence, remove correlation defer upon at last
            core_px->erase(filename_corre);
            ref->project_inode()->xattr_version++;
            ref->mark_dirty(ref->pre_dirty() + 1, mdlog->get_current_segment());
            dout(2) << "delete an obsolete correlation: name = " << filename_corre << dendl;
            #endif
           }   
        }
      }   
    ...
    if(in_prefethced.size() || v_dn_prefetched.size()){
      mdr->tracei_prefetched = in_prefethced;
      mdr->tracedn_prefetched = v_dn_prefetched;
      // mdr->phd_caps = corefs_phd_caps;
    }
  }
  #ifdef TIME_PHD
    utime_t phd_end = ceph_clock_now(g_ceph_context);
    dout(2) << "time_phd=" << phd_end - phd_start << dendl;
  #endif
  mdr->done_locking = true;
#endif // corefs prefetch
...
out:
  ...
  utime_t func_end = ceph_clock_now(g_ceph_context);
  // dout(2) << __func__ << " in rdblock_path_pin_ref lat=" << func_end - find_start << "s." << dendl;
...
void Server::handle_client_open(MDRequestRef& mdr)
{
  ...
  dout(2) << "[flag-open] " << flags <<" [mode-open] " << cmode << dendl;

  bool need_auth = !file_mode_is_readonly(cmode) || (flags & O_TRUNC);
  dout(7) << __func__ << " req's path1: " << req->get_path() << dendl;
  dout(7) << __func__ << " req's path2: " << req->get_path2() << dendl;
  dout(7) << __func__ << " open on " << req->get_filepath() << dendl;
  dout(7) << __func__ << " need_auth: " << need_auth << dendl;
  ...
  if (req->get_dentry_wanted()) {
    ...
    // corefs - prefetching dentry when opening
// #ifdef COREFS_PREFETCH
//     vector<CInode*> in_prefethced;
//     vector<vector<CDentry*>> v_dn_prefetched;
//     vector<CDentry*> dn_prefetched;
...
//     std::vector<CDentry*>::iterator pdn;
//     CInode *cin_prefetched;
//     map<string, bufferptr> *core_px = cur->get_projected_xattrs();
//     std::map<std::string, bufferptr>::iterator pxat = core_px->begin();
//     std::string filename_corre;
//     // int j = 0;
//     // int max = max_phd_items;
//     dout(2) << __func__ << " xattrs.size = " << core_px->size() << dendl;
//     for(; pxat != core_px->end(); pxat++){
//        // if(j >= max)
//        //   break;
//       filename_corre = pxat->first;
//       if(filename_corre[0] != '/'){
...
//       }
//       else{
//         if(corefs_get_dc_score(cur, filename_corre) >= SCORE_THRESHOLD){
//           dout(2) << __func__ << " prefetching inode and dentry.  Path." << filename_corre << dendl;
//           int r = mdcache->path_traverse(mdr, NULL, NULL, filepath(filename_corre.c_str()), &dn_prefetched, &cin_prefetched, MDS_TRAVERSE_FORWARD);
//           if (cin_prefetched != NULL){
//             // cap for cin_prefetched
//             mask = MAY_READ;
//             if (cmode & CEPH_FILE_MODE_WR)
//               mask |= MAY_WRITE;
//             if (!check_access(mdr, cin_prefetched, mask))
//               break;
//             if (cin_prefetched->is_file() || cin_prefetched->is_dir())
//             {
//               if (mdr->snapid == CEPH_NOSNAP)
//               {
//                 Capability *cap_phd = mds->locker->issue_new_caps(cin_prefetched, CEPH_FILE_MODE_RD, mdr->session, 0, req->is_replay());
//                 if(cap_phd)
//                   dout(2) << __func__ << " [corefs] open issued caps_prefetched " << ccap_string(cap_phd->pending())
//                       << " for " << req->get_source() << " on " << *cin_prefetched << dendl;
//               }
//               else{
//                 int caps_phd = ceph_caps_for_mode(CEPH_FILE_MODE_RD);
//                 dout(12) << __func__ << " [corefs] open issued IMMUTABLE SNAP caps " << ccap_string(caps_phd)
//                    << " for " << req->get_source()
//                    << " snapid " << mdr->snapid
//                    << " on " << *cin_prefetched << dendl;
//                 mdr->snap_caps = caps_phd;
//               }
//             }
...
//             #ifdef COREFS_DELETE
//             core_px->erase(filename_corre);
//             cur->project_inode()->xattr_version++;
//             // corefs_print_xattrs(cur);
//             cur->mark_dirty(cur->pre_dirty() + 1, mdlog->get_current_segment());
//             // cur->mark_dirty(cur->pre_dirty(), mdr->ls);
//             dout(2) << "delete an obsolete correlation: name = " << filename_corre << dendl;
//             #endif
//            }   
//         } 
...
void Server::handle_client_openc(MDRequestRef& mdr)
{
  ...
  dout(7) << __func__ << " open w/ O_CREAT on " << req->get_filepath() << dendl;
  dout(7) << __func__ << "req's path1:" << req->get_path() << dendl;
  dout(7) << __func__ << "req's path2:" << req->get_path2() << dendl;
  ...
  if (!(req->head.args.open.flags & O_EXCL)) {
    ...
    if (r == 0) {
      // it existed.
      dout(7) << "file is created, open now" << dendl;
      handle_client_open(mdr);
      return;
    }
    if (r < 0 && r != -ENOENT) {
...
void Server::handle_client_setxattr(MDRequestRef& mdr)
{
  ...
  if (!(flags & CEPH_XATTR_REMOVE)) {
    dout(10) << __func__ << " set xattr value" << dendl;
